...
namespace itk
{
  ...
  ::BuildModel()
  {
    ...
    MatrixType pcaMatrix = m_AlignedData / sqrt( (double)(this->GetNumberOfInputs()-1) );
  ...
  ::InitializeMatrix( MatrixType *matrix, unsigned int sampleIdx, OutputMeshPointer mesh ) const
  {
    ...
    typename OutputMeshType::PointsContainer::ConstIterator sample, mean;
    sample = meshPoints->Begin();
    mean = m_ProcrustesFilter->GetMean()->GetPoints()->Begin();
    for (unsigned int row=0; row<matrix->rows(); row+=3)
    {
      (*matrix)[row  ][sampleIdx] = sample.Value()[0] - mean.Value()[0];
      (*matrix)[row+1][sampleIdx] = sample.Value()[1] - mean.Value()[1];
      (*matrix)[row+2][sampleIdx] = sample.Value()[2] - mean.Value()[2];
      ++sample;
      ++mean;
    }
  }
