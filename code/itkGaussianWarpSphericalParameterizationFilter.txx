...
namespace itk
{
  ...
  {
    if (level < 0 || level > this->GetMaximumLevelOfDetail())
    {
  ...
  ::GenerateData()
  {
    ...
    if (m_NewInputSet)
    {
      this->InitializeOutput( this->GetInput() );
      m_NewInputSet = false;
    }
    double dir[2];
    ...
    ParameterizationType *input = this->GetInput();
    ParameterizationType *output = this->GetOutput();
    SphericalMapType *inputMap = this->GetInput()->GetSphericalMap();
